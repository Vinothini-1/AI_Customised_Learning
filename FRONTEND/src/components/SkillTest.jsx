...
const SkillTest = () => {
  ...
  const [analysis, setAnalysis] = useState(""); // Stores Gemini API response
      ...
      Advanced: [
        {
          ...
          {
            question: 'What is the secondary difference between generative and discriminative models?',
            options: [
  ...
  useEffect(() => {
    if (showResult && score !== undefined) { // Ensure score is defined
      const fetchAnalysis = async () => {
        const apiKey = "your api key"; // Replace with your valid API key
        const url = `https://generativelanguage.googleapis.com/v1/models/gemini-1.5-pro:generateContent?key=${apiKey}`;
  
        const requestBody = {
          contents: [{
            parts: [{
              text: `Based on the user's test results in Web development courses, with a score of ${score}, provide an analysis of their strengths, weaknesses, and learning goals.`
            }]
          }]
        };
  ...
  }, [showResult, score]); // Add 'score' to dependency array
